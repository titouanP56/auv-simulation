...
def generate_launch_description():
    ...
    # ── Gazebo simulation ────────────────────────────────────────────────
    world_file = os.path.join(pkg_auv_description, 'world', 'Bassin_ntnu.xml')
    ...
    # Odometry (Gz → ROS /odom — used as ground truth in MPC logs)
    ...
    # TF (Gz → ROS) — crucial to get odom -> bluerov2_realistic frame
    ...
    # IMU (Gz → ROS) — IMU topic is under the model name
    ...
    # ── DVL bridge: Gazebo DVL proto → /dvl/velocity_ros ────────────────
    # (TwistWithCovarianceStamped — consumed by the EKF)
    dvl_bridge = Node(
    ...
    # Fuses /imu/fixed (angular velocity) + /dvl/velocity_ros (body vels)
    # Publishes /odometry/filtered  ← consumed by the MPC
    ...
    # ── Static Transforms: Link world origin to odom origin ──────────────
    ...
    # This allows Gazebo to find meshes referenced via package:// (which it sees as model://)
