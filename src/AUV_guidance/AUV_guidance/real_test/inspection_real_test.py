...
class Phase3InspectionNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('test_enable_y', False)
        self.declare_parameter('test_enable_yaw', False)
        self.declare_parameter('test_enable_pitch', False)
    ...
    def _publish_thrusters(self, Fx: float, Fy: float, Fz: float, Mz: float, My: float = 0.0):
        if not self.get_parameter('test_enable_x').value:
            Fx = 0.0
        if not self.get_parameter('test_enable_y').value:
            Fy = 0.0
        if not self.get_parameter('test_enable_z').value:
            Fz = 0.0
        if not self.get_parameter('test_enable_yaw').value:
            Mz = 0.0
        if not self.get_parameter('test_enable_pitch').value:
            My = 0.0
